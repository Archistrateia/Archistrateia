...
namespace Archistrateia.Tests
{
    ...
    public class HexGridZoomTest
    {
        ...
        public void TestZoomFactorClamping()
        {
            ...
            HexGridCalculator.SetZoom(0.05f, _viewState); // Below minimum
            Assert.AreEqual(0.1f, _viewState.ZoomFactor, 0.001f, "Zoom should be clamped to minimum 0.1");
            
            HexGridCalculator.SetZoom(5.0f, _viewState); // Above maximum
            Assert.AreEqual(3.0f, _viewState.ZoomFactor, 0.001f, "Zoom should be clamped to maximum 3.0");
            
            HexGridCalculator.SetZoom(1.5f, _viewState); // Within range
            Assert.AreEqual(1.5f, _viewState.ZoomFactor, 0.001f, "Zoom should be set to 1.5");
        }
        ...
        public void TestZoomInAndOut()
        {
            ...
            Assert.AreEqual(1.2f, _viewState.ZoomFactor, 0.001f, "Zoom in should multiply by 1.2");
            
            // Test zoom out
            HexGridCalculator.ZoomOut(_viewState);
            Assert.AreEqual(1.0f, _viewState.ZoomFactor, 0.001f, "Zoom out should divide by 1.2");
        }
        ...
        {
            HexGridCalculator.SetZoom(1.0f, _viewState);
            var position1 = HexGridCalculator.CalculateHexPosition(1, 1, _viewState);
            
            HexGridCalculator.SetZoom(2.0f, _viewState);
            var position2 = HexGridCalculator.CalculateHexPosition(1, 1, _viewState);
        ...
        {
            HexGridCalculator.SetZoom(1.0f, _viewState);
            var vertices1 = HexGridCalculator.CreateHexagonVertices(_viewState);
            
            HexGridCalculator.SetZoom(1.5f, _viewState);
            var vertices2 = HexGridCalculator.CreateHexagonVertices(_viewState);
        ...
        public void TestCenteredPositionWithZoom()
        {
            ...
            HexGridCalculator.SetZoom(1.0f, _viewState);
            var centered1 = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, mapWidth, mapHeight, _viewState);
            
            HexGridCalculator.SetZoom(0.5f, _viewState);
            var centered2 = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, mapWidth, mapHeight, _viewState);
        ...
        {
            var originalZoom = _viewState.ZoomFactor;
            
            HexGridCalculator.SetZoom(1.5f, _viewState);
            Assert.AreEqual(1.5f, _viewState.ZoomFactor);
            
            HexGridCalculator.SetZoom(originalZoom, _viewState);
            Assert.AreEqual(originalZoom, _viewState.ZoomFactor);
        }
        ...
        public void TestScrollOffsetFunctionality()
        {
            ...
            HexGridCalculator.SetScrollOffset(testOffset, _viewState);
            Assert.AreEqual(testOffset, _viewState.ScrollOffset);
            
            // Test adding scroll offset
            var delta = new Vector2(25.0f, -10.0f);
            HexGridCalculator.AddScrollOffset(delta, _viewState);
            Assert.AreEqual(testOffset + delta, _viewState.ScrollOffset);
            
            // Test that scroll offset affects position calculation
            var viewportSize = new Vector2(800, 600);
            
            // Reset scroll offset to zero for baseline
            HexGridCalculator.SetScrollOffset(Vector2.Zero, _viewState);
            var positionWithZeroScroll = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            
            // Set scroll offset and verify position changes
            HexGridCalculator.SetScrollOffset(testOffset, _viewState);
            var positionWithScroll = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            ...
            HexGridCalculator.SetScrollOffset(originalScrollOffset, _viewState);
        }
        ...
        public void TestScrollDirectionIsCorrect()
        {
            ...
            HexGridCalculator.SetScrollOffset(Vector2.Zero, _viewState);
            var basePosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            
            // Test right scroll (positive X offset) - should move grid left (showing more right side)
            HexGridCalculator.SetScrollOffset(new Vector2(100.0f, 0.0f), _viewState);
            var rightScrollPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            ...
            HexGridCalculator.SetScrollOffset(new Vector2(0.0f, 100.0f), _viewState);
            var downScrollPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            ...
            HexGridCalculator.SetScrollOffset(new Vector2(-100.0f, 0.0f), _viewState);
            var leftScrollPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            ...
            HexGridCalculator.SetScrollOffset(new Vector2(0.0f, -100.0f), _viewState);
            var upScrollPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
        ...
        public void TestCurrentScrollingBehavior()
        {
            ...
            HexGridCalculator.SetScrollOffset(new Vector2(100.0f, 0.0f), _viewState);
            var positiveXPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            Assert.Less(positiveXPosition.X, basePosition.X, "Positive X offset moves grid left");
            
            // Test positive Y offset
            HexGridCalculator.SetScrollOffset(new Vector2(0.0f, 100.0f), _viewState);
            var positiveYPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            Assert.Less(positiveYPosition.Y, basePosition.Y, "Positive Y offset moves grid up");
            
            // Test negative X offset
            HexGridCalculator.SetScrollOffset(new Vector2(-100.0f, 0.0f), _viewState);
            var negativeXPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
            Assert.Greater(negativeXPosition.X, basePosition.X, "Negative X offset moves grid right");
            
            // Test negative Y offset
            HexGridCalculator.SetScrollOffset(new Vector2(0.0f, -100.0f), _viewState);
            var negativeYPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, 10, 10, _viewState);
        ...
        public void TestScrollBoundsWithZoom()
        {
            ...
            {
                HexGridCalculator.SetZoom(zoom, _viewState);
                
                // Get the scroll bounds for this zoom level
                var scrollBounds = HexGridCalculator.CalculateScrollBounds(viewportSize, mapWidth, mapHeight, _viewState);
                
                // Test scrolling to the top-left corner (negative scroll offset)
                HexGridCalculator.SetScrollOffset(new Vector2(-scrollBounds.X, -scrollBounds.Y), _viewState);
                var topLeftPosition = HexGridCalculator.CalculateHexPositionCentered(0, 0, viewportSize, mapWidth, mapHeight, _viewState);
                ...
                HexGridCalculator.SetScrollOffset(new Vector2(scrollBounds.X, scrollBounds.Y), _viewState);
                var bottomRightPosition = HexGridCalculator.CalculateHexPositionCentered(mapWidth - 1, mapHeight - 1, viewportSize, mapWidth, mapHeight, _viewState);
        ...
        public void TestScrollingNeededLogic()
        {
            ...
            var largeViewport = new Vector2(1200, 800);
            var smallGrid = new { Width = 10, Height = 5 };
            bool scrollingNeededSmall = HexGridCalculator.IsScrollingNeeded(largeViewport, smallGrid.Width, smallGrid.Height, _viewState);
            ...
            var smallViewport = new Vector2(400, 300);
            var largeGrid = new { Width = 30, Height = 20 };
            bool scrollingNeededLarge = HexGridCalculator.IsScrollingNeeded(smallViewport, largeGrid.Width, largeGrid.Height, _viewState);
            ...
            HexGridCalculator.SetZoom(3.0f, _viewState);
            bool scrollingNeededZoomed = HexGridCalculator.IsScrollingNeeded(largeViewport, smallGrid.Width, smallGrid.Height, _viewState);
            Assert.IsTrue(scrollingNeededZoomed, "Zoomed in grid should need scrolling");
            
            // Test case 4: Zoomed out grid (no scrolling needed)
            HexGridCalculator.SetZoom(0.5f, _viewState);
            bool scrollingNeededZoomedOut = HexGridCalculator.IsScrollingNeeded(largeViewport, smallGrid.Width, smallGrid.Height, _viewState);
            ...
            var gameGrid = new { Width = 20, Height = 10 };
            HexGridCalculator.SetZoom(1.0f, _viewState);
            bool scrollingNeededGame = HexGridCalculator.IsScrollingNeeded(gameViewport, gameGrid.Width, gameGrid.Height, _viewState);
        ...
        public void TestHomeKeyAlwaysWorks()
        {
            ...
            Assert.AreNotEqual(Vector2.Zero, _viewState.ScrollOffset);
            
            // Verify that scrolling is not needed for this setup
            bool scrollingNeeded = HexGridCalculator.IsScrollingNeeded(largeViewport, smallGrid.Width, smallGrid.Height, _viewState);
            ...
            HexGridCalculator.SetScrollOffset(Vector2.Zero, _viewState);
            Assert.AreEqual(Vector2.Zero, _viewState.ScrollOffset);
